    ...
    public async Task<IActionResult> TestWebhook([FromBody] ServerWebhooks_TestWebhookDTO request)
    {
        try
        {
            ...
            _logger.LogInformation("TestWebhook: EventType={EventType}, WebhookUrl={WebhookUrl}",
                request.EventType,
                string.IsNullOrWhiteSpace(request.WebhookUrl) ? "(empty)" : request.WebhookUrl.Substring(0, Math.Min(50, request.WebhookUrl.Length)).Replace("\r", "").Replace("\n", ""));
            ...
            {
                _logger.LogWarning("TestWebhook: Invalid URL format: {WebhookUrl}", request.WebhookUrl?.Replace("\r", "").Replace("\n", ""));
            ...
            {
                _logger.LogWarning("TestWebhook: URL is not a Discord webhook: {WebhookUrl}", request.WebhookUrl?.Replace("\r", "").Replace("\n", ""));
